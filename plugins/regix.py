...
async def restart_pending_forwads(bot, user):
    ...
    try:
        ...
        if (persistent_deduplication or datas['skip_duplicate']) and not dburi:
            await m.edit_text("You have enabled a deduplication feature, but you haven't set a MongoDB URI. Please add one in /settings.", disable_web_page_preview=True)
            await stop(client, user_id, bot_id)
            return
        ...
        keyword = datas['keywords']
        exten = datas['extensions']
        keywords = "|".join(keyword) if keyword else None
        extensions = "|".join(exten) if exten else None

        if not _bot:
            await msg_edit(m, "<code>Your bot was not found. Please re-add it in /settings!</code>", wait=True)
            return
        ...
        logger.error(f"Error restarting task for user {user_id}: {e}", exc_info=True)
        await db.rmve_frwd(user_id, bot_id)
